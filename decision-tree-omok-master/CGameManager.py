...
class CGameManager(object):
    def __init__(self, Screen):    
        ...
        try:
            ...
            self.m_MainMenu.AppendMenu("font/나눔고딕Bold.ttf", 34, 'play alone'.encode('utf-8'), self.m_SoloPlay_Pos, self.m_MenuOn_Color, self.m_MenuOff_Color)
            self.m_MainMenu.AppendMenu("font/나눔고딕Bold.ttf", 34, '둘이하기'.encode('cp949'), self.m_MultiPlay_Pos, self.m_MenuOn_Color, self.m_MenuOff_Color)
            ...
            self.m_MainMenu.AppendMenu("font/나눔고딕Bold.ttf", 34, '게임종료'.encode('cp949'), self.m_QuitGame_Pos, self.m_MenuOn_Color, self.m_MenuOff_Color)
            ...
            self.m_ChoiceStonMenu.AppendMenu("font/나눔고딕Bold.ttf", 34, 'black'.encode('cp949'), self.m_BlackSton_Pos, self.m_MenuOn_Color, self.m_MenuOff_Color)
            self.m_ChoiceStonMenu.AppendMenu("font/나눔고딕Bold.ttf", 34, 'white(not supported)'.encode('cp949'), self.m_WhiteSton_Pos, self.m_MenuOn_Color, self.m_MenuOff_Color)
            self.m_ChoiceStonMenu.AppendMenu("font/나눔고딕Bold.ttf", 34, 'back'.encode('cp949'), self.m_ChoiceSton_Back_Pos, self.m_MenuOn_Color, self.m_MenuOff_Color)

            self.m_Onemore_Pos = (233, 240)
            self.m_StopGame_Pos = (233, 302)
            self.m_ResultMenu = CMenuManager.CMenuManager(Screen)
            self.m_ResultMenu.AppendMenu("font/나눔고딕Bold.ttf", 34, 'one more'.encode('cp949'), self.m_Onemore_Pos, self.m_MenuOn_Color, self.m_MenuOff_Color)
            self.m_ResultMenu.AppendMenu("font/나눔고딕Bold.ttf", 34, 'stop playing'.encode('cp949'), self.m_StopGame_Pos, self.m_MenuOn_Color, self.m_MenuOff_Color)
            ...
            self.m_MultiMenu.AppendMenu("font/나눔고딕Bold.ttf", 34, '방만들기'.encode('cp949'), self.m_Host_Pos, self.m_MenuOn_Color, self.m_MenuOff_Color)
            self.m_MultiMenu.AppendMenu("font/나눔고딕Bold.ttf", 34, '접속하기'.encode('cp949'), self.m_Connect_Pos, self.m_MenuOn_Color, self.m_MenuOff_Color)
            self.m_MultiMenu.AppendMenu("font/나눔고딕Bold.ttf", 34, '뒤로'.encode('cp949'), self.m_Multi_Back_Pos, self.m_MenuOn_Color, self.m_MenuOff_Color)

            self.m_WaitClient_Back = (512, 8)
            self.m_WaitClientMenu = CMenuManager.CMenuManager(Screen)
            self.m_WaitClientMenu.AppendMenu("font/나눔고딕Bold.ttf", 34, 'Back'.encode('cp949'), self.m_WaitClient_Back, self.m_MenuOn_Color, self.m_MenuOff_Color)

            self.m_Connect_Back = (512, 8)
            self.m_ConnectMenu = CMenuManager.CMenuManager(Screen)
            self.m_ConnectMenu.AppendMenu("font/나눔고딕Bold.ttf", 34, 'Back'.encode('cp949'), self.m_Connect_Back, self.m_MenuOn_Color, self.m_MenuOff_Color)
    ...
    def GetFavorablePos(self, Type):
        ...
        max_many= []
        max_many_loc=[]
        # 여기를 정렬한다음에 위에서 세개, 다섯개 끊고, 그중에 하나랜덤으로 내보내는 걸로 바꾸자.
        if max_many_mode:
            for y in range(self.map_size):
                for x in range(self.map_size):
                    inserted = False
        ...
        # 돌 동안 한번도 크지 않았으면 끝에 끼워넣어주는거 해야함.
    ...
    def SimulateUserStone(self,Type):
        if self.m_UserTurn==True:
            ...
            if total== 0:
                ...
                if is_random: # 전체 랜덤모드 켜져있으면
                    pos = self.TotalRandom(Type)
                else:
                    if is_ML_mode: # Decision Tree 기반으로 다음 수 선택
                        current_sit_rep = SitRepTurn.SitRepTurn(self.idGame,self.idTurn,self.m_Map,Type)
                        # 이번 턴 보고서를 aiserver에 보내고 다음 턴 order를 받아온다.
                        next_order = self.aiserver.giveNextMove(current_sit_rep)
                        if next_order is False: # aiserver에서 이런 수를 처음 보면
                            pos = self.TempUserAI(Type) # 기존 최고점수 자리에 두기 알고리즘 작동.
                        else:
        ...
        if (e.button == 1):
            ...
            #########################################################
                ...
                if (self.m_ChoiceStonMenu.GetActivationMenu(e.pos) == 0):
                    ...
                    self.idTurn=0
                    self.StartSoloPlay(self.m_UserSton) # 유저 돌색깔 1로 정하고 게임시작, gamestatus =2 됨
